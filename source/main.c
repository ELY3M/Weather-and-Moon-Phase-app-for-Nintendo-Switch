...
int main()
{
	...
	moonage = getmoonage(); 
	snprintf(moonicon, sizeof(moonicon)+1, "romfs:/gfx/moon%.0f.png", round(moonage));
	moonImage = IMG_Load(moonicon);
	moonTexture = SDL_CreateTextureFromSurface(renderer, moonImage);
	...
	while (appletMainLoop())
	{
		...
		renderTexture(moonTexture, renderer, 0, 0, 100, 200, 300, 300);
		...
		//moonage = getmoonage(); 
		//SDL_DrawTextf(renderer, smfont, 3, SCREEN_HEIGHT + 200, CYAN, "Moon Age: %.2f", moonage);
		//SDL_DrawTextf(renderer, smfont, 3, SCREEN_HEIGHT + 250, CYAN, "Moon Age: %.0f", round(moonage));
